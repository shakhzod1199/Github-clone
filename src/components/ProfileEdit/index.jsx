...
const index = (props) => {
    const { user: { avatar_url, login, followers, following } } = props;

    return (
        <div className='profileEdit position-absolute'>
            <div className='avatar_name  col-md'>
                <Link className='col-md-12 '>
                    <img src={avatar_url} alt="" className='rounded-circle w-100' />
                </Link>
                <p className='editLogin'>{login}</p>
            </div>
            <button className="btn btn-outline-secondary w-100">Edit profile</button>
            <span className='follow d-flex gap-4 mt-3 align-items-center'>
                <Link to='/followers'>
                    <p>
